...
function matchesQuery(slot: DraftSlot, query: string): boolean {
  ...
  return [name, team, position, category, country].some((field) =>
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="relative block w-full sm:max-w-md">
          <span className="sr-only">Cerca un talento rivelato</span>
          <input
            type="search"
            value={query}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/2 px-6 py-10 text-center">
          ...
          <p className="mt-1 text-sm text-zinc-600">Prova con un altro nome o ruolo.</p>
        </div>
      ) : (
